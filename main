...
def showWeather(data1):
    ...
    newScreen.geometry('300x300+550+250')
    ...
    desc = tk.Label(newScreen, text=data1[5], font=('Inter', 15, 'bold'))
    desc.place(x=50, y=20)

    location = tk.Label(newScreen, text=f"Location : {data1[0]}", font=('Inter', 13))
    location.place(x=20, y=70)

    temperature = tk.Label(newScreen, text=f"Temperature : {data1[1]}°C", font=('Inter', 13))
    temperature.place(x=20, y=110)

    current_humidiy = tk.Label(newScreen, text=f"Humidity: {data1[2]}%", font=('Inter', 13))
    current_humidiy.place(x=20, y=150)

    current_pressure = tk.Label(newScreen, text=f"Pressure : {data1[3]}", font=('Inter', 13))
    current_pressure.place(x=20, y=190)

    close_button = tk.Button(newScreen, text='Close', command=lambda: [city.delete(0, tk.END), newScreen.destroy()],
                             font=('Inter', 15, 'bold'), bg='#121212', fg="#fafef8")
...
api_key = "14deb4dc2aedd179bfeed9dfaa252baa"
...
bg.pack()
tit_le = tk.Label(root, text="Enter City Below", font=('Inter', 20, 'bold'), bg='#1f252f', fg='#fafef8')
tit_le.place(x=120, y=79)
city = tk.Entry(root, font=('Inter', 15))
city.place(x=120, y=140)
city.focus()
cta_button = tk.Button(root, text='Find Weather', command=findWeather)
cta_button.place(x=189, y=186)
root.mainloop()
